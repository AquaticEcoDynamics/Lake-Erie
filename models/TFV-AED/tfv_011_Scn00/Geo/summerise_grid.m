function summerise_grid(filename,outfile)
...
fprintf(fid,'Filename,Number of Cells,Total Area (m2),Min Cell Area (m2),Max Cell Area (m2), Mean Cell Area (m2), Min Depth (mAHD), Max Depth (mAHD), Mean Depth (mAHD)\n');

fprintf(fid,'%s,%10.4f,%10.4f,%10.4f,%10.4f,%10.4f,%10.4f,%10.4f,%10.4f\n',...
    filename,length(Z),sum(A),min(A),max(A),mean(A),min(Z),max(Z),mean(Z));
fclose(fid);

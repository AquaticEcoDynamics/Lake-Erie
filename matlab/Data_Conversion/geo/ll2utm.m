...
%!!!!!!!!!! NOT TO BE USED NEAR THE POLES ie. ABOVE 84 OR BELOW -80 DEGREES LATITUDE !!!!!!!!!!!
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*
% 
% vectorised by JN
%
...
%   datum vector(optional) :   CLARKE_1866_DATUM = 1;
%                       GRS_80_DATUM = 2;
...
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*
  ...
  % this section not vectorized as it is assumed the grid zone for the first point will be applied to the entire data set
  
  if nargin < 3                                                            % changed from 4 by JN
    datum = WGS_84_DATUM;
  end
  ...
  phi = latitude * pi / 180.0;                               %vector
  lambda = longitude * pi / 180.0;                           %vector
 
  %/* Figure out the UTM zone, as well as lambda0 */

  [grid_zone, lambda0] =  get_grid_zone (longitude, latitude);
  phi0 = zeros(length(longitude),1);                        

  
%   !!!!!!!!!!!! WARNING THIS SECTION OF CODE IS NOT APPLIED AS IT IS ASSUMED WBM WON'T WORK NEAR THE POLES !!!!!!!!!!!!!!!
...
%   else
    ...
    ep2 = e2 / (1.0 - e2) * ones(length(longitude),1);

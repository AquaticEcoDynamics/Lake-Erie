function plottfv_polygon_color(conf)
...
switch conf
    ...
    case 'test'
        config_polygon_CEWH_Reporting_test;
...
%         dewnr_leg = 0;
        ...
        for mod = 1:length(ncfile)
            ...
            if isfield(data,'date')
                if mod == 1
                    %
                    fig = fillyy(data(mod).date,data(mod).pred_lim_ts(1,:),data(mod).pred_lim_ts(2*nn-1,:),ncfile(mod).colour{1},0.2);hold on
                    set(fig,'DisplayName',[ncfile(mod).legend,' (Surface Range)']);
                    hold on
                    
                    for plim_i=2:(nn-1)
                        fig2 = fillyy(data(mod).date,data(mod).pred_lim_ts(plim_i,:),data(mod).pred_lim_ts(2*nn-plim_i,:),ncfile(mod).colour{1},0.4);
...
%                                         'linestyle',':','markersize',12,'displayname','Field');hold on
%                                     dewnr_leg = 1;
%                                 else
%                                     fp = plot(xdata_d,ydata_d,'color',def.fieldcolour{1},'marker',def.fieldsymbol{1},...
%                                         'linestyle',':','markersize',12,'HandleVisibility','off');hold on
%                                 end
%                                 %                     else
%                                 %                         if epa_leg == 0
...
%                 ydata = data(mod).pred_lim_ts(3,:);
                ...
                plot(xdata,ydata,ncfile(mod).colour{1},'linewidth',1.5,'DisplayName',[ncfile(mod).legend,' (Surface Median)']);hold on
                
                if addBottom

                    
                    for plim_i=2:(nn-1)
                        fig2XX = fillyy(data(mod).date,data(mod).pred_lim_ts_b(plim_i,:),data(mod).pred_lim_ts_b(2*nn-plim_i,:),ncfile(mod).colour{2},0.4);
                        set(fig2XX,'HandleVisibility','off');
                        uistack(fig2XX,'bottom');
                    end

                    figXX = fillyy(data(mod).date,data(mod).pred_lim_ts_b(1,:),data(mod).pred_lim_ts_b(2*nn-1,:),ncfile(mod).colour{2},0.2);hold on
                    set(figXX,'DisplayName',[ncfile(mod).legend,' (Bottom Range)']);
                    uistack(figXX,'bottom');
                    hold on

                    [xdata,ydata] = tfv_averaging(data(mod).date,data(mod).pred_lim_ts_b(3,:),def);
                    data(mod)
                    plot(xdata,ydata,ncfile(mod).colour{2},'linewidth',1.5,'DisplayName',[ncfile(mod).legend,' (Bottom Median)']);hold on
                end
...
%                     end
        ...
        if exist('filetype','var')
            if strcmpi(filetype,'png');
                print(gcf,'-dpng',regexprep(finalname,'.eps','.png'),'-opengl','-r300');
                print(gcf,'-dpng',regexprep(finalname,'.eps','.png'),'-opengl','-r300');
            else

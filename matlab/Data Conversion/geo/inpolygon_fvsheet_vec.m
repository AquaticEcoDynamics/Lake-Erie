function [in, on] = inpolygon_fvsheet_vec(x,y,xv,yv)
%
% WORKS OFF THE MAGIC OF inpolygon BUT AS THE POINTS ARE KNOWN TO FALL
% WITHIN THE X BOUNDS OF THE POLYGON LINE 83 IS MODIFIED ACCORDINGLY
%
%
%
...
%   $Revision: 1.14.4.10 $  $Date: 2010/11/17 11:29:27 $
...
if xrange < min_safe_limit || yrange < min_safe_limit
   warning('MATLAB:inpolygon:ModelingWorld',...
...
if xrange > max_safe_limit || yrange > max_safe_limit
     warning('MATLAB:inpolygon:ModelingWorld',...
       'The results from INPOLYGON may not be reliable.\nThe size of the polygon is approaching the upper limit\nof what can be handled with reasonable accuracy.');
end
inbounds = find(mask);
x = x(mask);
y = y(mask);

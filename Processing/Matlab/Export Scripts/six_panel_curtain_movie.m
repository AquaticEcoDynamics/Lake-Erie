...
ncfile = 'D:\Simulations\Erie\Lake Erie\TFVAED2\tfv_002_2013_2dwind_aed2_2018\Output\erie__aed.nc';;
geofile = 'D:\Simulations\Erie\Lake Erie\TFVAED2\tfv_002_2013_2dwind_aed2_2018\Input\erie_geo.nc';
linefile = 'erie_1.xy';
...
outdir = 'Example_Output\six_panel_curtain_movie\';
...
for TL = 100:4:length(timesteps)
    ...
    for n = 1 : (N - 1)
        ...
        SAL{n} = data2.WQ_OXY_OXY(i3:i3+NL-1);
        DO{n} = data2.WQ_NIT_NIT(i3:i3+NL-1);
        TCHLA{n} = data.WQ_PHY_CHLOR(i3:i3+NL-1);
        NIT{n} = data.WQ_PHY_CRYPT(i3:i3+NL-1);
        AMM{n} = data.WQ_PHY_DINOF(i3:i3+NL-1);
        FRP{n} = data.WQ_PHY_CYANO(i3:i3+NL-1);
    ...
    text(0.05,0.1,'NO_3','Color','w','units','normalized','fontsize',16,'FontWeight','bold');
